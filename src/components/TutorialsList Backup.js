...
const TutorialsList = (props) => {
  ...
  const AddTourDetails = () => {
    axios
      .post(addTourUrl, tourData)
      .then((response) => {
        setStatus(response && response.data.status);
        setMessage(response && response.data.message); 
        tourStatus.status=='success' ? setShowModal(false) : setShowModal(true);
        setShowModal(false);
      })
      .catch((error) => {
        setShowModal(true);
  ...
  const UpdateTourDetails = () => {
    const updateUrl = tourUrl+"/"+showDeleteModal.new_id;
    axios
      .post(updateUrl, tourData)
      .then((response) => {
        setStatus(response && response.data.status);
        setMessage(response && response.data.message); 
        tourStatus.status=='success' ? setShowModal(false) : setShowModal(true);
  ...
  return (
    <>
      {tourStatus && tourStatus=='success' &&(
        <div class="flex">
          <div class="m-auto">
            <div class="bg-blue-300 rounded-lg border-gray-300 border p-3 shadow-lg">
